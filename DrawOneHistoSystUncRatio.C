...
int main( int argc, const char* argv[] ){
  ...
  TH1F* mcerr = (TH1F*)h_SumAll->Clone("mcerr");
  for (int ibin = 1; ibin < h_lumiBand->GetNbinsX()+1; ibin++){
    double dnErr = 0.;
    double upErr = 0.;
    ...
    mcerr->SetBinContent(ibin,0.);
    if (h_SumAll->GetBinContent(ibin) != 0) {

      double N = histos[0].at(Data)->GetBinContent(ibin);

      if (N < h_SumAll->GetBinContent(ibin)) ratio->SetBinError(ibin, (histos[0].at(Data)->GetBinErrorUp(ibin) / h_SumAll->GetBinContent(ibin)) );
      else ratio->SetBinError(ibin, (histos[0].at(Data)->GetBinErrorLow(ibin) / h_SumAll->GetBinContent(ibin)) );
      ratio->SetBinContent(ibin, N / h_SumAll->GetBinContent(ibin));
      if (histos[0].at(Data)->GetBinContent(ibin) == 0) {
        ratio->SetBinContent(ibin, 0.000001);
        ratio->SetBinError(ibin, 1.8 / h_SumAll->GetBinContent(ibin));
      }
      double raterrup = upErr/h_SumAll->GetBinContent(ibin);
      double raterrdn = dnErr/h_SumAll->GetBinContent(ibin);
      mcerr->SetBinContent(ibin, 1.+((raterrup-raterrdn)/2.));
      mcerr->SetBinError(ibin, (raterrup+raterrdn)/2.);
    }
...
//   cout<<mystack->GetHistogram()->GetMaximum()<<'\t'<<mystack->GetHistogram()->GetMinimum()<<endl;
 ...
 //   text2->SetY(0.9);
  ...
  mcerr->SetMarkerStyle(20);
  mcerr->SetMarkerSize(0);
  mcerr->SetFillColor(kRed);
  ...
  pad2->cd();
  mcerr->Draw("E2");
  ratio->Draw("P0 E0 X0 same");
  
  mcerr->SetMinimum(0.25);
  mcerr->SetMaximum(1.75);

  TAxis * xAxis = mcerr->GetXaxis();
  xAxis->SetTitle(xTitle);
  xAxis->SetTitleFont(42);
  ...
  canv->SaveAs(inDir+outName+"_SystUnc_Ratio.pdf");
  canv->SaveAs(inDir+outName+"_SystUnc_Ratio.png");
  canv->SaveAs(inDir+outName+"_SystUnc_Ratio.C");
