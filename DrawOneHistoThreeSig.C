...
int main( int argc, const char* argv[] ){
  ...
  bool blind = false;
  if (argc > 12) if (atoi(argv[12])>0) blind = true;
  ...
  for (unsigned int isam = 0; isam < NSAMPLES; isam++){
    ...
    //std::cout<<"Getting "+inDir+fileName+":/"+inPref+"/Mu/"+inName+"_"+inPref+"_"+allNames[isam]+"_Mu"<<std::endl;
  ...
  selectedHistos.push_back(histos.at(WJets_Pt100to250_HF)); 
  selectedHistos.push_back(histos.at(WJets_Pt250to400_HF)); 
  selectedHistos.push_back(histos.at(WJets_Pt400to600_HF)); 
  selectedHistos.push_back(histos.at(WJets_Pt600toInf_HF)); 
  selectedHistos.push_back(histos.at(WJets_Pt100to250_LF)); 
  selectedHistos.push_back(histos.at(WJets_Pt250to400_LF)); 
  selectedHistos.push_back(histos.at(WJets_Pt400to600_LF)); 
  selectedHistos.push_back(histos.at(WJets_Pt600toInf_LF)); 
  ...
  selectedUnc.push_back(vbUnc); //WJets_Pt100to250
  selectedUnc.push_back(vbUnc); //WJets_Pt250to400
  selectedUnc.push_back(vbUnc); //WJets_Pt400to600
  selectedUnc.push_back(vbUnc); //WJets_Pt600toInf
  selectedUnc.push_back(vbUnc); //WJets_Pt100to250
  selectedUnc.push_back(vbUnc); //WJets_Pt250to400
  selectedUnc.push_back(vbUnc); //WJets_Pt400to600
  selectedUnc.push_back(vbUnc); //WJets_Pt600toInf
...
//   TLegend * leg = new TLegend(0.67, 0.58, 0.89, 0.93, "","brNDC");
  ...
  if (not blind) leg->AddEntry(histos.at(Data), "Data", "ep");
  ...
  if (useQCD){
    ...
    qcdBG->Scale(scale);
    std::cout<<"QCD contribution = "<<qcdBG->Integral()/h_SumAll->Integral()<<std::endl;
    mystack->Add(qcdBG);
  }

  //Vector Boson BG
  //TH1F* vbBG = (TH1F*)histos.at(WJets)->Clone("vbBG");
  TH1F* vbBG = (TH1F*)histos.at(WJets_Pt600toInf_LF)->Clone("vbBG");
  vbBG->Add(histos.at(WJets_Pt100to250_LF));
  vbBG->Add(histos.at(WJets_Pt250to400_LF));
  vbBG->Add(histos.at(WJets_Pt400to600_LF));
  TH1F* vbhfBG = (TH1F*)histos.at(WJets_Pt600toInf_HF)->Clone("vbhfBG");
  vbhfBG->Add(histos.at(WJets_Pt100to250_HF));
  vbhfBG->Add(histos.at(WJets_Pt250to400_HF));
  vbhfBG->Add(histos.at(WJets_Pt400to600_HF));
  ...
  //double wjbkg = vbBG->Integral();
  //std::cout<<"W+jets contribution = "<<scale*(wjbkg+vbhfBG->Integral())/h_SumAll->Integral()<<std::endl;
  vbBG->Add(histos.at(ZJets));
  //double zjbkg = vbBG->Integral()-wjbkg;
  //std::cout<<"Z+jets contribution = "<<scale*zjbkg/h_SumAll->Integral()<<std::endl;
  vbBG->Add(histos.at(WW));
  vbBG->Add(histos.at(WZ));
  vbBG->Add(histos.at(ZZ));
  //std::cout<<"VV contribution = "<<scale*(vbBG->Integral()-wjbkg-zjbkg)/h_SumAll->Integral()<<std::endl;
  ...
  vbhfBG->SetFillColor(kBlue);
  vbhfBG->SetLineColor(kBlack);
  leg->AddEntry(vbhfBG, "W#rightarrowl#nu (HF)", "f");
  vbhfBG->Scale(scale);
  mystack->Add(vbhfBG);

  //Top BG
  TH1F* topBG = (TH1F*)histos.at(TTbar)->Clone("topBG");
  //double ttbkg = topBG->Integral();
  //std::cout<<"TTbar contribution = "<<scale*ttbkg/h_SumAll->Integral()<<std::endl;
  ...
  //std::cout<<"Single top contribution = "<<scale*(topBG->Integral()-ttbkg)/h_SumAll->Integral()<<std::endl;
...
//   cout<<mystack->GetHistogram()->GetMaximum()<<'\t'<<mystack->GetHistogram()->GetMinimum()<<endl;
  ...
  if (not blind) histos.at(Data)->Draw("P E X0 same");  
  ...
  TLatex* text1;
  if (label==0) text1 = new TLatex(1.570061,23.08044,"#scale[0.75]{#it{Preliminary}                           35.9 fb^{-1}  (13 TeV)}");
  else text1 = new TLatex(1.570061,23.08044,"#scale[0.75]{#it{(Unpublished)}                           35.9 fb^{-1}  (13 TeV)}");
  text1->SetNDC();
 ...
 //   text2->SetY(0.9);
  ...
  bool chkfin = inPref.Contains("Final");
  bool chksoft = inPref.Contains("Soft");
  bool chkhard = inPref.Contains("Hard");
  ...
  if (chkfin) text3.AddText("Type A + Type B");
  else if (chksoft) text3.AddText("Type A");
  else if (chkhard) text3.AddText("Type B");
  ...
  pad2->Draw();
  pad2->cd();
  if (not blind) ratio->Draw("hist");
  else ratio->Draw("axis");
  ...
  if (useQCD) {
    if (label==0) {
      ...
      canv->SaveAs(inDir+outName+fileSuff+"_wQCD_Pub.pdf");
      canv->SaveAs(inDir+outName+fileSuff+"_wQCD_Pub.png");
      canv->SaveAs(inDir+outName+fileSuff+"_wQCD_Pub.C");
    }
    else {
    ...
    if (label==0) {
      ...
      canv->SaveAs(inDir+outName+fileSuff+"_Pub.pdf");
      canv->SaveAs(inDir+outName+fileSuff+"_Pub.png");
      canv->SaveAs(inDir+outName+fileSuff+"_Pub.C");
    }
    else {
