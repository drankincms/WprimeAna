...
for isuf in range(len(suffix)):
    locdir = sufbase+suffix[isuf]+'_'+logdir
    eosdir = 'root://cmseos.fnal.gov//store/user/drankin/WprimeToTB/'+sufbase+suffix[isuf]+'_'+logdir
    ...
    for i in range(len(samples)):
        ...
        csh_templ_file    = open(rel_base+"/src/UserCode/drankin/WprimeAna/Analyzercsh.templ.toppt")
        ...
        for line in csh_templ_file:
            ...
            line=line.replace('SUFFIX',sufbase+suffix[isuf])
            ...
            line=line.replace('ANALYZER','WprimeTopPt')
            line=line.replace('OUTNAME','topPtOutFile')
            line=line.replace('OPTSTR',opts[isuf])
