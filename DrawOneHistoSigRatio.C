#ifndef DrawOneHistoSigRatio_cxx
#define DrawOneHistoSigRatio_cxx
...
//   TLegend * leg = new TLegend(0.67, 0.58, 0.89, 0.93, "","brNDC");
  ...
  TH1F* vbBG = (TH1F*)histos.at(WJets_Pt600toInf_HF)->Clone("vbBG");
  vbBG->Add(histos.at(WJets_Pt100to250_HF));
  vbBG->Add(histos.at(WJets_Pt250to400_HF));
  vbBG->Add(histos.at(WJets_Pt400to600_HF));
  vbBG->Add(histos.at(WJets_Pt100to250_LF));
  vbBG->Add(histos.at(WJets_Pt250to400_LF));
  vbBG->Add(histos.at(WJets_Pt400to600_LF));
  vbBG->Add(histos.at(WJets_Pt600toInf_LF));
  ...
  histos.at(Wprime2000Right)->SetLineStyle(2);
...
//   cout<<mystack->GetHistogram()->GetMaximum()<<'\t'<<mystack->GetHistogram()->GetMinimum()<<endl;
 ...
 //   text2->SetY(0.9);
  ...
  yAxis2->CenterTitle();
  yAxis2->SetRangeUser(0.5,1.5);

  gPad->SetGridy(1);

  TString outName = "h_"+inPref+"_"+inName;
  if (inSuff != "All") outName += "_"+inSuff+"_Ratio";  
  ...
  if (useQCD) {
    if (label==0) {
      ...
      canv->SaveAs(inDir+outName+"_wQCD_Unpub.pdf");
      canv->SaveAs(inDir+outName+"_wQCD_Unpub.png");
      canv->SaveAs(inDir+outName+"_wQCD_Unpub.C");
    }
  }
  else {
    if (label==0) {
      ...
      canv->SaveAs(inDir+outName+"_Unpub.pdf");
      canv->SaveAs(inDir+outName+"_Unpub.png");
      canv->SaveAs(inDir+outName+"_Unpub.C");
    }
  }
