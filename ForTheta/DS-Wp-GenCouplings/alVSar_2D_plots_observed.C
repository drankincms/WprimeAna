...
void alVSar_2D_plots_observed (std::string chan) {
  ...
  char line[80];
  string header;
  float dum[35],mx[35],y[35],yerr[35];
  int num=0;
  int numal=0;
  int numar=0;
  ...
  for (int il=0; il<11; il++){
    for (int ir=0; ir<11; ir++){
      ...
      numal=il;
      numar=ir;
      sprintf(filename, "%s%d%s%d%s","bayesian_limits_observed_",numal,mid,numar,".txt");
      //cout << filename << endl;
      num++;
      ...
      while(!inFile.eof() ){
        ...
        inFile >> mx[i] >> y[i] >> yerr[i];
        if (il==0 && ir==0) cout<<mx[i]<<" "<<y[i]<<" "<<yerr[i]<<endl;
  ...
  c1->Update();
  //c1->Print(Form("contour_arVSal-%s_observed.pdf",chan.c_str()));
  //c1->Print(Form("contour_arVSal-%s_observed.png",chan.c_str()));
