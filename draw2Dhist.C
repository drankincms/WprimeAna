...
void plot(std::string chan, std::string sample)
{
	...
	htmp = (TH2D*)file1->Get(Form("h_%sPt_vs_Eta_%s",chan.c_str(),sample.c_str()));
	//htmp->RebinY(4);
	//htmp->RebinX(4);
	htmp->SetTitle("");
	...
	can->SaveAs(Form("DistPtEta_%s_%s.pdf",chan.c_str(),sample.c_str()));
