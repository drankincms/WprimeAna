#ifndef WprimeAnalyzer_Slim_Opt_cxx
#define WprimeAnalyzer_Slim_Opt_cxx
...
//   int MIN_HT =0;
  ...
  if (opt=="wjm") {
    //WJHFSF = 1.41668;//HT bins
    //WJLFSF = 0.804747;//HT bins
    WJHFSF = 1.91597;//Pt bins
    WJLFSF = 0.574564;//Pt bins
  }
  if (opt=="wjp") {
    //WJHFSF = 1.804;//HT bins
    //WJLFSF = 0.62325;//HT bins
    WJHFSF = 2.30949;//Pt bins
    WJLFSF = 0.392293;//Pt bins
  }

  cout<<"Processing "<<sampleName<<'\t'<<fileName<<'\t'<<opt<<endl;
  ...
  std::string dataFileUp("DataPU_Jan04_2016_plus.root");//pileup up
  std::string dataFileDown("DataPU_Jan04_2016_minus.root");//pileup down
  //std::string dataFile("DataPU_Jul21_2016BCD.root");
  //std::string dataFileUp("DataPU_Jul21_2016BCD_plus.root");//pileup up
  //std::string dataFileDown("DataPU_Jul21_2016BCD_minus.root");//pileup down
  std::string genHistName("histo");
  std::string dataHistName("pileup");
  fexists(generatedFile,true);
  fexists(dataFile,true);
  fexists(dataFileUp,true);
  fexists(dataFileDown,true);
  ...
  LumiReWeighting lumiReWeightingDown(generatedFile, dataFileDown, genHistName, dataHistName);
...
//   char syncName[300];
  ...
  if (opt=="base") { 
    if (b_RealData) sprintf(name,"aSlimOutFile_base_%s_%s_%lx.root", argv[2], argv[4], rawtime);
//  else            sprintf(name,"aSlimOutFile_%s_%lx.root",    argv[2], rawtime);
    else            sprintf(name,"aSlimOutFile_base_%s_%s.root", argv[2], argv[4]);
  }
  else {
    if (b_RealData) sprintf(name,"aSlimOutFile_%s_%s_%s_%lx.root",  argv[5], argv[2], argv[4], rawtime);
//  else            sprintf(name,"aSlimOutFile_%s_%lx.root",    argv[2], rawtime);
    else            sprintf(name,"aSlimOutFile_%s_%s_%s.root",  argv[5], argv[2], argv[4]);
  }
  ...
  vector <StandardHistos*> Ex0BTags_Soft_std_h(2);
  vector <StandardHistos*> Ex1BTags_Soft_std_h(2);
  vector <StandardHistos*> Ex2BTags_Soft_std_h(2);
  vector <StandardHistos*> GE1BTags_Soft_std_h(2);
  vector <StandardHistos*> GE0BTags_Soft_std_h(2);

  vector <StandardHistos*> Ex0BTags_Hard_std_h(2);
  vector <StandardHistos*> Ex1BTags_Hard_std_h(2);
  vector <StandardHistos*> Ex2BTags_Hard_std_h(2);
  vector <StandardHistos*> GE1BTags_Hard_std_h(2);
  vector <StandardHistos*> GE0BTags_Hard_std_h(2);
  ...
  initHistoDirTwoChannels(outFile, Ex0BTags_Soft_std_h,           "Ex0BTags_Soft",          sampleName, sScale);
  ...
  initHistoDirTwoChannels(outFile, GE1BTags_Soft_std_h,           "GE1BTags_Soft",          sampleName, sScale);
  initHistoDirTwoChannels(outFile, GE0BTags_Soft_std_h,           "GE0BTags_Soft",          sampleName, sScale);

  initHistoDirTwoChannels(outFile, Ex0BTags_Hard_std_h,           "Ex0BTags_Hard",          sampleName, sScale);
  ...
  initHistoDirTwoChannels(outFile, GE1BTags_Hard_std_h,           "GE1BTags_Hard",          sampleName, sScale);
  initHistoDirTwoChannels(outFile, GE0BTags_Hard_std_h,           "GE0BTags_Hard",          sampleName, sScale);
  ...
  set <long long> checkDuplicatesLumi;
  int nDup = 0;
  long long RUNPREF = 1000 * 1000; 
  RUNPREF *= 1000 * 1000;

  long long MAXEVTNUM = 1; 
  ...
  //for (Long64_t il = 0; il < 5; il++){
    ...
    else if (opt=="pup")                teve->weight = lumiReWeightingUp.weight(nti);
    else if (opt=="pum")                teve->weight = lumiReWeightingDown.weight(nti);
...
//     else                teve->weight = treeClass->weight_PU;
    ...
    if (opt=="mup" or opt=="mum") {
      double maxwe = 1.;
      double minwe = 1.;
      int maxID, minID;
      for (unsigned int it = 0; it<treeClass->LHEweightids->size(); it++) {
        if (treeClass->LHEweightids->at(it)>1010) break;
        int tmpID = treeClass->LHEweightids->at(it);
        if ((tmpID<10 or tmpID>1000) && tmpID!=6 && tmpID!=8 && tmpID!=1006 && tmpID!=1008) {
          if(treeClass->LHEweights->at(it)>maxwe) {maxwe = treeClass->LHEweights->at(it); maxID = tmpID;}
          if(treeClass->LHEweights->at(it)<minwe) {minwe = treeClass->LHEweights->at(it); minID = tmpID;}
        }
        if (opt=="mum" && (treeClass->LHEweightids->at(it)==1005 || treeClass->LHEweightids->at(it)==5)) teve->weight *= treeClass->LHEweights->at(it);//correlated down
        if (opt=="mup" && (treeClass->LHEweightids->at(it)==1009 || treeClass->LHEweightids->at(it)==9)) teve->weight *= treeClass->LHEweights->at(it);//correlated up
      }
      //if (maxwe>1. && maxID!=9 && maxID!=1009) std::cout<<"WARNING: Correlated Q2 up not largest variation!!!! ID = "<<maxID<<std::endl;
      //if (minwe<1. && minID!=5 && minID!=1005) std::cout<<"WARNING: Correlated Q2 down not smallest variation!!!! ID = "<<minID<<std::endl;
      //if (opt=="mup") teve->weight *= maxwe;
      //if (opt=="mum") teve->weight *= minwe;
    }
    ...
    if ((sampleName=="TTbar" or sampleName=="TTbar_scaleup" or sampleName=="TTbar_scaledown") and opt!="tpp") {
    ...
    if (sampleName=="TTbar" and opt=="tpm") {
      //teve->weight *= 0.947944;
      //teve->lim_weight *= 0.947944;
    }
    ...
    if (treeClass->goodLeptons.size() == 1){
      ...
      if (treeClass->goodLeptons.size() == 0) Nempty++;
      continue;
    }
    ...
    if (teve->lepton1->isMuon() and (opt=="mrp" or opt=="mrm")) {
      TRandom3 randnum;
      randnum.SetSeed(teve->lepton1->lv.Phi()*1e4);
      float sign = (opt=="mrp" ? 1. : -1.);
      double temppt = teve->lepton1->lv.Pt()*(1.+(sign*randnum.Gaus(0.,0.1)));
    ...
    if (teve->lepton1->isMuon() and (opt=="msp" or opt=="msm")) {
      float sign = (opt=="msp" ? 1. : -1.);
      double temppt = 1./((1./teve->lepton1->lv.Pt())+sign*(teve->lepton1->charge)*(fabs(teve->lepton1->lv.Eta()) >= 1.2 ? 0.0001 : 0.00005));
      double metcx = (teve->lepton1->lv.Pt()-temppt)*TMath::Cos(teve->lepton1->lv.Phi());
      double metcy = (teve->lepton1->lv.Pt()-temppt)*TMath::Sin(teve->lepton1->lv.Phi());
      teve->met_x += metcx;
      teve->met_y += metcy;
    ...
    //if (teve->lepton1->charge < 0.) continue;
...
//------------------- CHECKING FOR DUPLICATES (?) -------------------
      ...
      if (treeClass->event > MAXEVTNUM) MAXEVTNUM = treeClass->event;
      long long dupCheck = treeClass->run * RUNPREF + treeClass->event;
      long long dupCheckLumi = treeClass->lumi * RUNPREF + treeClass->event;
      if (checkDuplicates.find(dupCheck) != checkDuplicates.end()){
        if (checkDuplicatesLumi.find(dupCheckLumi) != checkDuplicatesLumi.end()){
	  nDup++;
	  continue;
        }
      }
      else {
        checkDuplicates.insert(dupCheck);
        checkDuplicatesLumi.insert(dupCheckLumi);
      }
    ...
    for (unsigned int ij = 0; ij < teve->vGoodAK4Jets.size(); ij++){
      ...
      if (teve->vGoodAK4Jets.at(ij)->csvTag[CsvSet] > 0) teve->nBTags++;
    ...
    if (sampleName.BeginsWith("WJets")) {
      ...
      /*for (unsigned int ij = 0; ij < teve->vGoodAK4Jets.size(); ij++){
        ...
        if (opt=="jpp"){
          teve->weight *= (1.204 - 0.0008301*jetpt + 0.0000002940*jetpt*jetpt);
          teve->lim_weight *= (1.204 - 0.0008301*jetpt + 0.0000002940*jetpt*jetpt);
        } else if (opt=="jpm"){
          teve->weight *= std::max(0.1, 0.985 - 0.0005296*jetpt + 0.00000000005*jetpt*jetpt);
          teve->lim_weight *= std::max(0.1, 0.985 - 0.0005296*jetpt + 0.00000000005*jetpt*jetpt);
        } else {
    ...
    if (teve->vGoodAK4Jets.at(0)->csvTag[CsvSet] > 0 || teve->vGoodAK4Jets.at(1)->csvTag[CsvSet] > 0) firstbtag = true;
    if (teve->vGoodAK4Jets.at(0)->csvTag[CsvSet] > 0 && teve->vGoodAK4Jets.at(1)->csvTag[CsvSet] > 0) first2btag = true;
    ...
    if (sampleIndex == Data and teve->lvWprime.M()>4000.){
	cout<<"-----------------------------------------------------"<<endl;
    ...
    if (teve->lvTop.Pt() > 650 && finalcuts && ((teve->vGoodAK4Jets.at(0)->lv)+(teve->vGoodAK4Jets.at(1)->lv)).Pt() > 700) finalcuts_hard = true;
    ...
    if (finalcuts_hard) fillHistosTwoChannels(GE0BTags_Hard_std_h, teve);
    if (!finalcuts_hard && finalcuts) fillHistosTwoChannels(GE0BTags_Soft_std_h, teve);
    if (finalcuts) fillHistosTwoChannels(GE0BTags_Final_std_h, teve);
    if ( !firstbtag) {
      fillHistosTwoChannels(Ex0BTags_std_h, teve);
      if (finalcuts_hard) fillHistosTwoChannels(Ex0BTags_Hard_std_h, teve);
      if (!finalcuts_hard && finalcuts) fillHistosTwoChannels(Ex0BTags_Soft_std_h, teve);
    ...
    if ( firstbtag && !first2btag) {
      ...
      if (finalcuts_hard) fillHistosTwoChannels(Ex1BTags_Hard_std_h, teve);
      if (!finalcuts_hard && finalcuts) fillHistosTwoChannels(Ex1BTags_Soft_std_h, teve);
      if (finalcuts) fillHistosTwoChannels(Ex1BTags_Final_std_h, teve);
    }
    if ( firstbtag ) {
      fillHistosTwoChannels(GE1BTags_std_h, teve);
      if (finalcuts_hard) fillHistosTwoChannels(GE1BTags_Hard_std_h, teve);
      if (!finalcuts_hard && finalcuts) fillHistosTwoChannels(GE1BTags_Soft_std_h, teve);
      if (finalcuts) fillHistosTwoChannels(GE1BTags_Final_std_h, teve);
    }
    if ( first2btag ) {
      fillHistosTwoChannels(Ex2BTags_std_h, teve);
      if (finalcuts_hard) fillHistosTwoChannels(Ex2BTags_Hard_std_h, teve);
      if (!finalcuts_hard && finalcuts) fillHistosTwoChannels(Ex2BTags_Soft_std_h, teve);
  ...
  if (not teve->realData){
    ...
    scaleHistosTwoChannels(Ex0BTags_Soft_std_h);
    scaleHistosTwoChannels(Ex1BTags_Soft_std_h);
    scaleHistosTwoChannels(Ex2BTags_Soft_std_h);
    scaleHistosTwoChannels(GE1BTags_Soft_std_h);
    scaleHistosTwoChannels(GE0BTags_Soft_std_h);

    scaleHistosTwoChannels(Ex0BTags_Hard_std_h);
    scaleHistosTwoChannels(Ex1BTags_Hard_std_h);
    scaleHistosTwoChannels(Ex2BTags_Hard_std_h);
    scaleHistosTwoChannels(GE1BTags_Hard_std_h);
    scaleHistosTwoChannels(GE0BTags_Hard_std_h);
  }

  cout<<"Number of duplicate events: "<<nDup<<endl;
  if (teve->realData) cout<<"Maximum event number: "<<MAXEVTNUM<<endl;
  cout<<"Number of empty events: "<<Nempty<<endl;
