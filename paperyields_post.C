...
void paperyields_post()
{

	TFile *file2 = new TFile("Run2016_Feb27_Slim/outFile.root");
	...
	std::vector<double> wjscale, ttscale, errors;
	//el1A, el2A, el1B, el2B, mu1A, mu2A, mu1B, mu2B
	wjscale.push_back(0.949635526372/0.98166349544920194394);
	ttscale.push_back(0.899932152575);
	errors.push_back(62.1129458354);
	wjscale.push_back(0.914896139941/0.89530151683314835368);
	ttscale.push_back(0.893201396043);
	errors.push_back(33.612651747);
	wjscale.push_back(0.947404890726/0.81069549139524960887);
	ttscale.push_back(0.878941932649);
	errors.push_back(23.2098229151);
	wjscale.push_back(0.942682577069/0.92715231788079470199);
	ttscale.push_back(0.893064605614);
	errors.push_back(6.70635417535);
	wjscale.push_back(0.980327740651/0.96940354600521731441);
	ttscale.push_back(0.861648395676);
	errors.push_back(69.9555040015);
	wjscale.push_back(0.922794101168/0.89184368412518242257);
	ttscale.push_back(0.88074169224);
	errors.push_back(30.0005434426);
	wjscale.push_back(0.907690123049/0.99791643820594363417);
	ttscale.push_back(0.853437788228);
	errors.push_back(22.0473591044);
	wjscale.push_back(0.937193845703/0.96082779009608277901);
	ttscale.push_back(0.855441334358);
	errors.push_back(8.94125066929);

	std::vector<string> bcat;
	bcat.push_back("Ex1BTags_Soft");
	bcat.push_back("Ex2BTags_Soft");
	bcat.push_back("Ex1BTags_Hard");
	bcat.push_back("Ex2BTags_Hard");
	...
	std::cout<<"\\caption{Number of selected data, signal, and background events. The expectation for signal and background events is computed corresponding to an integrated luminosity of "<<luminosity<<" fb$^{-1}$. ''Type A'' refers to the additional cuts of $250<p_{T}^{top}<650$, $350<p_{T}^{j_1+j_2}<700$, and $100<m_{top}<250$. ''Type B' refers to the additional cuts of $650<p_{T}^{top}$, $700<p_{T}^{j_1+j_2}$, and $100<m_{top}<250$}"<<std::endl;
	...
	std::cout<<" & \\mc{2}{c}{Type A} \\vline& \\mc{2}{c}{Type B} \\vline& \\mc{2}{c}{Type A} \\vline& \\mc{2}{c}{Type B}\\\\"<<std::endl;
	...
	for (int k=0; k<sample.size(); k++) {
		...
		for (int j=0; j<chan.size(); j++) {
			for (int i=0; i<bcat.size(); i++) {
				htmp = (TH1F*)file2->Get(Form("%s/%s/mWprime_Limits_%s_%s_%s",bcat[i].c_str(),chan[j].c_str(),bcat[i].c_str(),sample[k].c_str(),chan[j].c_str()));
				double error;
				if ((strncmp(sample[k].c_str(),"WJ",2)==0) && sample[k]!="WJets_Pt600toInf_LF" && sample[k]!="WJets_Pt600toInf_HF") {
					...
					tmpsum[i+(bcat.size()*j)] *= scale;
					if (strncmp(sample[k].c_str(),"T",1)==0) tmpsum[i+(bcat.size()*j)] *= ttscale[i+(bcat.size()*j)];
					else tmpsum[i+(bcat.size()*j)] *= wjscale[i+(bcat.size()*j)];
					tmpsum[i+(bcat.size()*j)] += 0.5;
					std::cout <<" & \\mc{1}{c}{"<<(int)tmpsum[i+(bcat.size()*j)]<<"}";
					if (i%2==1 && !(i==(bcat.size()-1) && j==(chan.size()-1))) std::cout<<"\\vline";
	...
	for (int j=0; j<chan.size(); j++) {
		for (int i=0; i<bcat.size(); i++) {
			...
			std::cout<<" & \\mc{1}{c}{"<<totsum[i+(bcat.size()*j)]<<"$\\pm$"<<(int)(errors[i+(bcat.size()*j)]+0.5)<<"}";
			if (i%2==1 && !(i==(bcat.size()-1) && j==(chan.size()-1))) std::cout<<"\\vline";
		}
	}
	std::cout<<"\\\\"<<std::endl;
	...
	for (int j=0; j<chan.size(); j++) {
		for (int i=0; i<bcat.size(); i++) {
			hdata = (TH1F*)file2->Get(Form("%s/%s/mWprime_Limits_%s_Data_%s",bcat[i].c_str(),chan[j].c_str(),bcat[i].c_str(),chan[j].c_str()));
