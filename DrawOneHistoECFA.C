...
int main( int argc, const char* argv[] ){
  ...
  for (unsigned int isam = 0; isam < NSAMPLES; isam++){
    ...
    if (!useQCD and (isam == ZJets_HT100to200 or isam == ZJets_HT200to400 or isam == ZJets_HT400to600 or isam == ZJets_HT600to800 or isam == ZJets_HT800to1200 or isam == ZJets_HT1200to2500 or isam == ZJets_HT2500toInf or isam == WJets_HT100to200_LF or isam == WJets_HT200to400_LF or isam == WJets_HT400to600_LF or isam == WJets_HT600to800_LF or isam == WJets_HT800to1200_LF or isam == WJets_HT1200to2500_LF or isam == WJets_HT2500toInf_LF or isam == WJets_HT100to200_HF or isam == WJets_HT200to400_HF or isam == WJets_HT400to600_HF or isam == WJets_HT600to800_HF or isam == WJets_HT800to1200_HF or isam == WJets_HT1200to2500_HF or isam == WJets_HT2500toInf_HF or isam == WJets or isam == QCD_HT200to300 or isam == QCD_HT300to500 or isam == QCD_HT500to700 or isam == QCD_HT700to1000 or isam == QCD_HT1000to1500 or isam == QCD_HT1500to2000 or isam == QCD_HT2000toInf or isam == QCD_Pt_5to10 or isam == QCD_Pt_10to15 or isam == QCD_Pt_15to30 or isam == QCD_Pt_30to50 or isam == QCD_Pt_50to80 or isam == QCD_Pt_80to120 or isam == QCD_Pt_120to170 or isam == QCD_Pt_170to300 or isam == QCD_Pt_300to470 or isam == QCD_Pt_470to600 or isam == QCD_Pt_600to800 or isam == QCD_Pt_800to1000 or isam == QCD_Pt_1000to1400 or isam == QCD_Pt_1400to1800 or isam == QCD_Pt_1800to2400
    or isam == TTbar_scaleup
    or isam == TTbar_scaledown
    or isam == Tbar_s
    or isam == Wprime800Right
    or isam == Wprime900Right
  ...
  selectedHistos.push_back(histos.at(WJets_HT600to800)); 
  selectedHistos.push_back(histos.at(WJets_HT800to1200)); 
  selectedHistos.push_back(histos.at(WJets_HT1200to2500)); 
  selectedHistos.push_back(histos.at(WJets_HT2500toInf)); 
...
//   TLegend * leg = new TLegend(0.67, 0.58, 0.89, 0.93, "","brNDC");
  ...
  if (useQCD){
    ...
    qcdBG->Scale(scale);
    qcdBG->Scale(100000.);//not adjusted for 14 TeV yet!!
    mystack->Add(qcdBG);
  }

  //Vector Boson BG
  //TH1F* vbBG = (TH1F*)histos.at(WJets)->Clone("vbBG");
  TH1F* vbBG = (TH1F*)histos.at(WJets_HT2500toInf)->Clone("vbBG");
  vbBG->Add(histos.at(WJets_HT100to200));
  vbBG->Add(histos.at(WJets_HT200to400));
  vbBG->Add(histos.at(WJets_HT400to600));
  vbBG->Add(histos.at(WJets_HT600to800));
  vbBG->Add(histos.at(WJets_HT800to1200));
  vbBG->Add(histos.at(WJets_HT1200to2500));
  vbBG->Scale(1.082);
  ...
  vbBG->Add(histos.at(ZJets),1.085);
  vbBG->Add(histos.at(WW),1.110);
  vbBG->Add(histos.at(WZ),1.115);
  vbBG->Add(histos.at(ZZ),1.114);
  ...
  topBG->Scale(1.183);
  topBG->Add(histos.at(T_s),1.044);
  topBG->Add(histos.at(T_t),1.138);
  topBG->Add(histos.at(T_tW),1.177);
  //topBG->Add(histos.at(Tbar_s));
  topBG->Add(histos.at(Tbar_t),1.152);
  topBG->Add(histos.at(Tbar_tW),1.177);
  ...
  histos.at(Wprime2500Right)->SetLineStyle(2);
  ...
  histos.at(Wprime3000Right)->SetLineStyle(3);
  ...
  histos.at(Wprime3500Right)->Scale(20*scale);
  histos.at(Wprime3500Right)->SetLineStyle(4);
  histos.at(Wprime3500Right)->SetLineWidth(2);
  histos.at(Wprime3500Right)->SetLineColor(kBlack);
  leg->AddEntry(histos.at(Wprime3500Right), "W'_{R}, m=3.5 TeV #times 20", "l");

  histos.at(Wprime4000Right)->Scale(20*scale);
  histos.at(Wprime4000Right)->SetLineStyle(3);
  histos.at(Wprime4000Right)->SetLineWidth(2);
  histos.at(Wprime4000Right)->SetLineColor(kBlack);
  leg->AddEntry(histos.at(Wprime4000Right), "W'_{R}, m=4.0 TeV #times 20", "l");

  if (inName=="mWprime_Limits") {
    histos.at(Wprime2500Right)->Scale(0.043750);
    histos.at(Wprime3000Right)->Scale(0.014806);
    histos.at(Wprime3500Right)->Scale(0.0053946);
    histos.at(Wprime4000Right)->Scale(0.0021051);
  }
  else {
    histos.at(Wprime2500Right)->Scale(1.312);
    histos.at(Wprime3000Right)->Scale(1.377);
    histos.at(Wprime3500Right)->Scale(1.435);
    histos.at(Wprime4000Right)->Scale(1.477);
  }
...
//   }  
  ...
  else           mystack->SetMinimum(100);
  
//   if (inPref != "SS_2L_ZQV_2J") hmax *= 10;

  if (histos.at(Wprime2500Right)->GetMaximum()>hmax) hmax = histos.at(Wprime2500Right)->GetMaximum();
...
//   cout<<mystack->GetHistogram()->GetMaximum()<<'\t'<<mystack->GetHistogram()->GetMinimum()<<endl;
  ...
  histos.at(Wprime3500Right) ->Draw("hist same");
  histos.at(Wprime4000Right) ->Draw("hist same");
  ...
  TLatex* text1;
  if (label==0) text1 = new TLatex(1.570061,23.08044,"#scale[0.85]{CMS Simulation   L = 3000 fb^{-1}, #sqrt{s} = 14 TeV}");
  ...
  TAxis * xAxis1 = mystack->GetXaxis();
  xAxis1->SetTitle(xTitle);
  xAxis1->SetTitleFont(42);
  xAxis1->SetTitleSize(0.047);
  xAxis1->SetTitleOffset(1.0);
  xAxis1->SetLabelFont(42);
  xAxis1->SetLabelSize(0.035);
  xAxis1->SetLabelOffset(0.007);
 ...
 //   text2->SetY(0.9);
  ...
  TString logset;
  if (yLog == 0) logset = "_Lin";
  else logset = "_Log";

  if (useQCD) {
    if (label==0) {
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_wQCD.pdf");
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_wQCD.png");
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_wQCD.C");
    }
    else {
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_wQCD_Unpub.pdf");
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_wQCD_Unpub.png");
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_wQCD_Unpub.C");
    }
  }
  else {
    if (label==0) {
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA.pdf");
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA.png");
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA.C");
    }
    else {
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_Unpub.pdf");
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_Unpub.png");
      canv->SaveAs(inDir+outName+fileSuff+logset+"_ECFA_Unpub.C");
    }
  }
