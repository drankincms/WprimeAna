#ifndef DrawWJetsShape_cxx
#define DrawWJetsShape_cxx
...
int main( int argc, const char* argv[] ){
  ...
  TFile *filebase = TFile::Open(inDir+"outFile.root");
  filebase->cd();

  histArray[0][0] = (TH1F*)gDirectory->Get(inDir+"outFile.root:/"+inPref+"/Mu/"+inName+"_"+inPref+"_"+allNames[0]+"_Mu");
  histArray[0][1] = (TH1F*)gDirectory->Get(inDir+"outFile.root:/"+inPref+"/El/"+inName+"_"+inPref+"_"+allNames[0]+"_El");

  if (rebin != 1){
    histArray[0][0]->Rebin(rebin);
    histArray[0][1]->Rebin(rebin);
  }

  histArray[0][2] = (TH1F*)histArray[0][0]->Clone("histArray_"+allNames[0]);
  histArray[0][2]->Add(histArray[0][1]);
    
  if      (inSuff == "Mu") histos.push_back(histArray[0][0]);
  else if (inSuff == "El") histos.push_back(histArray[0][1]);
  else if (inSuff == "All")  histos.push_back(histArray[0][2]);
  else{
    cout<<"Invalid suffix"<<endl;
    return 1;
  }

  TString fileName = "outFile"+fileSuff+".root";
  ...
  for (unsigned int uh = 0; uh < histos.size(); uh++){
	...
	histos.at(uh)->SetBinError(ub, 1.0);
      }
    }
  ...
  double mcUnc = sqrt (0.05 * 0.05 + //Trigger
                       0.05 * 0.05 + //Lepton Efficiency
                       0.06 * 0.06); //Luminosity
...
//   TLegend * leg = new TLegend(0.67, 0.58, 0.89, 0.93, "","brNDC");
  ...
  if (useQCD) leg->AddEntry(histos.at(Data), "Data - Top - QCD", "ep");
  else leg->AddEntry(histos.at(Data), "Data - Top", "ep");
  ...
  leg->AddEntry(vbBG, "W#rightarrowl#nu + Z/#gamma*#rightarrowl^{+}l^{-} + VV", "f");
  vbBG->Scale(scale);
  mystack->Add(vbBG);
  ...
  topBG->Scale(scale);
  //mystack->Add(topBG);
  histos.at(Data)->Add(topBG,-1.);

  TH1F* h_lumiBand = (TH1F*) vbBG->Clone("h_lumiBand");
  ...
  for (int ibin = 1; ibin < h_lumiBand->GetNbinsX()+1; ibin++){
    ...
    for (unsigned int ih = 1; ih < 11; ih++){// for specific case, ZZ,WW,WZ,inclusive ZJets, HT binned WJets
...
//   cout<<mystack->GetHistogram()->GetMaximum()<<'\t'<<mystack->GetHistogram()->GetMinimum()<<endl;
 ...
 //   text2->SetY(0.9);
  ...
  TString outName = "h_"+inPref+"_"+inName+fileSuff;
  if (inSuff != "All") outName += "_"+inSuff;  

  canv->SaveAs(inDir+outName+"_WJetsShape.pdf");
  canv->SaveAs(inDir+outName+"_WJetsShape.png");
  canv->SaveAs(inDir+outName+"_WJetsShape.C");
