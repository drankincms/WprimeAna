#include "../interface/HcalLaserEventFilter2012.h"
...
HcalLaserEventFilter2012::HcalLaserEventFilter2012(const std::string & eventFileName)
{
  ...
  if (verbose_) cout << "HCAL laser event list from file "<<eventFileName<<endl;
  readEventListFile(eventFileName);
  std::sort(EventList_.begin(), EventList_.end());
  if (verbose_) cout<<" A total of "<<EventList_.size()<<" listed HCAL laser events found in given run range"<<endl;
